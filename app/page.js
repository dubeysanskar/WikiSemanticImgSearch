...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/stats').then(r => r.json()).then(d => setUserCount(d.totalUsers || 0)).catch(() => {});
  }, []);
  ...
  return (<>
    ...
    {showHistory && token && <HistoryPanel token={token} onClose={() => setShowHistory(false)} onSearch={(q) => {setQuery(q);doSearch(q);}} />}
