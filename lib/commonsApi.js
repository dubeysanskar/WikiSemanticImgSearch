...
function getUserAgent(username) {
  const u = username || process.env.MEDIAWIKI_USERNAME || 'WikiSemanticSearch';
  return `WikiSemanticImgSearch/1.0 (https://github.com/dubeysanskar/WikiSemanticImgSearch; MediaWiki user: ${u})`;
}

async function apiRequest(params, username) {
  ...
  const resp = await fetch(url.toString(), {
    headers: { 'User-Agent': getUserAgent(username), Accept: 'application/json' },
