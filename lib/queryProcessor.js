...
export function decomposeQuery(query) {
  ...
  // Complex query — decompose while keeping context
  ...
  // Core subjects = keywords that aren't geo/time/env modifiers
  const subjects = keywords.filter(
    (k) => !geoParts.includes(k) && !timeParts.includes(k) && !envParts.includes(k) && k.length > 2
  );

  const coreSubject = subjects.slice(0, 2).join(' '); // e.g. "temple"
  const subQueries = new Set();

  // 1. Full cleaned query (capped at 5 words for keyword search)
  if (keywords.length <= 5) subQueries.add(cleaned);

  // 2. Core subject + each modifier (keeps context)
  // "indian temple", "temple monsoon", "19th century temple"
  if (coreSubject) {
    for (const geo of geoParts.slice(0, 2)) {
      subQueries.add(`${geo} ${coreSubject}`);
    }
    for (const env of envParts.slice(0, 2)) {
      subQueries.add(`${coreSubject} ${env}`);
    }
    for (const time of timeParts.slice(0, 2)) {
      subQueries.add(`${time} ${coreSubject}`);
    }

    // 3. Core subject + geo + env combined (most specific)
    // "indian temple monsoon"
    if (geoParts.length > 0 && envParts.length > 0) {
      subQueries.add(`${geoParts[0]} ${coreSubject} ${envParts[0]}`);
    }
    if (geoParts.length > 0 && timeParts.length > 0) {
      subQueries.add(`${timeParts[0]} ${geoParts[0]} ${coreSubject}`);
    }

    // 4. Just the core subject as broad fallback
    subQueries.add(coreSubject);
  }

  // 5. Multi-word noun phrases from the query (keeps natural phrasing)
